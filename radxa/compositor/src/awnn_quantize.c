/*
 * awnn_quantize.c — Quantization/dequantization for awnn
 *
...
static double awnn_copy_sign(double number, double sign)
{
...
static double awnn_rint(double x)
{
#define _EPSILON 1e-8
    ...
    {
        return awnn_copy_sign(math_floorf(MATH_ABS(x) + 0.5f), x);
    }
...
vip_int32_t fp32_to_dfp(const float in, const signed char fl, const vip_enum type)
{
    ...
    if(fl > 0)
    {
        data = (vip_int32_t)awnn_rint(in * (float)(1 << fl));
    }
    else
    {
        data = (vip_int32_t)awnn_rint(in * (1.0f / (float)(1 << -fl)));
    }
    ...
    )
{
    ...
    double min_range;
    type_get_range(type, &max_range, &min_range);
    data = (vip_int32_t)(awnn_rint(in / scale) + zero_point);
...
static unsigned short fp32_to_bfp16_rtne(float in)
{
    ...
    vip_uint32_t lsb = (fp32 >> 16) & 1;
    vip_uint32_t rounding_bias = 0x7fff + lsb;

    if (0x7FC00000 == (vip_uint32_t)in) {
        out = 0x7fc0;
    }
    else {
        fp32 += rounding_bias;
...
unsigned short fp32_to_fp16(float in)
{
    ...
    t1 = (fp32 & 0x80000000u) >> 16;
    t2 = (fp32 & 0x7F800000u) >> 13;
    t3 = (fp32 & 0x007FE000u) >> 13;
