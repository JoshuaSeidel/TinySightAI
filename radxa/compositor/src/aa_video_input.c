/*
 * aa_video_input.c — Receive tapped AA video frames from aa-proxy
 *
 * Internal design:
 *
 *   Two frame buffers (ping-pong):
 *     buf[0] and buf[1] — each AA_VIDEO_MAX_FRAME bytes.
 *
 *   The receiver thread writes into buf[write_idx], then atomically
 *   swaps write_idx and sets g_new_frame = 1.
 *
 *   aa_video_input_get_frame() copies the pointer and codec under a mutex,
 *   clears g_new_frame, and returns.
 *
 *   Frame read protocol (identical to carplay_input):
 ...
 *     4. Store in buffer, signal consumer.
 *
 *   The aa-proxy connects to us (compositor listens, aa-proxy is the client).
 *   On aa-proxy reconnect we accept again and continue.
 */
...
static void *aa_video_recv_thread(void *arg)
{
    (void)arg;

    while (g_running) {
        /* Wait for aa-proxy to connect */
        printf("aa_video_input: waiting for aa-proxy on %s\n",
               AA_VIDEO_SOCK_PATH);

        int fd = accept(g_server_fd, NULL, NULL);
        if (fd < 0) {
            if (g_running) perror("aa_video_input: accept");
            continue;
        }
        printf("aa_video_input: aa-proxy connected (video tap)\n");
        ...
        while (g_running) {
            ...
            if (frame_len == 0 || frame_len > AA_VIDEO_MAX_FRAME) {
                fprintf(stderr,
                        "aa_video_input: bad frame length %u, dropping connection\n",
                        frame_len);
                break;
            }
        ...
        close(fd);
        printf("aa_video_input: aa-proxy disconnected (video tap)\n");
    }
...
int aa_video_input_init(void)
{
    ...
    g_buf[0] = malloc(AA_VIDEO_MAX_FRAME);
    g_buf[1] = malloc(AA_VIDEO_MAX_FRAME);
    if (!g_buf[0] || !g_buf[1]) {
        fprintf(stderr, "aa_video_input: buffer allocation failed\n");
        free(g_buf[0]);
        free(g_buf[1]);
    ...
    unlink(AA_VIDEO_SOCK_PATH);

    g_server_fd = socket(AF_UNIX, SOCK_STREAM, 0);
    if (g_server_fd < 0) {
        perror("aa_video_input: socket");
        return -1;
    }
    ...
    strncpy(addr.sun_path, AA_VIDEO_SOCK_PATH, sizeof(addr.sun_path) - 1);

    if (bind(g_server_fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        perror("aa_video_input: bind");
        close(g_server_fd);
        g_server_fd = -1;
        return -1;
    }
    if (listen(g_server_fd, 1) < 0) {
        perror("aa_video_input: listen");
        close(g_server_fd);
        g_server_fd = -1;
        return -1;
    }
    ...
    if (pthread_create(&g_thread, NULL, aa_video_recv_thread, NULL) != 0) {
        perror("aa_video_input: pthread_create");
        g_running = 0;
        close(g_server_fd);
        g_server_fd = -1;
        return -1;
    ...
    printf("aa_video_input: listening on %s\n", AA_VIDEO_SOCK_PATH);
    return 0;
}
...
void aa_video_input_destroy(void)
{
    ...
    unlink(AA_VIDEO_SOCK_PATH);
    printf("aa_video_input: destroyed\n");
}
