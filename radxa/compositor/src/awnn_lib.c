/*
 * awnn_lib.c — VIPLite convenience wrapper for NPU inference
 *
 * Vendored from ZIFENG278/ai-sdk (examples/libawnn_viplite/)
 * Modified: removed Android <log/log.h> (logging now in awnn_internal.h)
 */
...
static void dump_param(Awnn_params_t *param, const char *ioname, int index) {
    ...
    ss += sprintf(buffer + ss, ", data_format=%d, name=%s, elements=%u",
                  param->vip_param.data_format, param->name, param->elements);
    switch(param->vip_param.quant_format) {
        ...
        case VIP_BUFFER_QUANTIZE_TF_ASYMM:
            ss += sprintf(buffer + ss, ", scale=%f, zero_point=%d\n",
                          param->vip_param.quant_data.affine.scale,
                          param->vip_param.quant_data.affine.zeroPoint);
            break;
        default:
...
static vip_status_e load_param(Awnn_Context_t *info) {
    ...
    for (i = 0; i < info->output_count; i++) {
        ...
        if (param->data_format == VIP_BUFFER_FORMAT_UINT8) {
            ...
            for (j = 0; j < 256; j++) {
                info->quantize_maps[i][j] = uint8_to_fp32(j,
                    param->quant_data.affine.zeroPoint, param->quant_data.affine.scale);
            }
        } else if (param->data_format == VIP_BUFFER_FORMAT_INT8) {
            info->quantize_maps[i] = (float *)malloc(sizeof(float) * 256);
            if (param->quant_format == VIP_BUFFER_QUANTIZE_DYNAMIC_FIXED_POINT) {
                for (j = 0; j < 256; j++) {
                    info->quantize_maps[i][j] = int8_to_fp32(j,
                        param->quant_data.dfp.fixed_point_pos);
                }
                ...
                for (j = 0; j < 256; j++) {
                    ...
                    info->quantize_maps[i][j] = affine_to_fp32(src_value,
                        param->quant_data.affine.zeroPoint, param->quant_data.affine.scale);
                }
            }
        }
